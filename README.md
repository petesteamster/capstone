...
- We performed extensive statistical analysis in our analysis of how R and Python MICE handle consinuous data. We compared density curves visually and numerically. To do the numeric comparisions, we used techniques from functional data analysis (the R package FDASRVF) to calcuculate a metric used to measure how simular two density curves are to each other. We compared the density curves of the Python and the R imputations to the actual values they replaced. We had enough data to perform a t-test. The results are shown in the table below.    
